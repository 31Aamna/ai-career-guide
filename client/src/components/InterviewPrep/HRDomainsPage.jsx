...
import { hrDomains } from '../../data/interviewData';

const HRDomainsPage = ({ onSelectDomain, onBack }) => {
  return (
    <div>
      <div style={{textAlign:'center', marginBottom:'40px'}}>
        <h2 style={{fontSize:'28px'}}>HR Interview Modules</h2>
      ...
      <div className="grid-4" style={{gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))'}}>
        ...
        {/* {hrDomains.map((domain) => (
          <Card key={domain.id} onClick={() => onSelectDomain(domain.id)} style={{cursor:'pointer', minHeight:'150px'}}>
            <div style={{width:'40px', height:'40px', background:'#F0FDF4', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', marginBottom:'15px'}}>{domain.icon}</div>
            <h4>{domain.id}</h4>
            <p style={{fontSize:'12px', color:'var(--text-light)'}}>{domain.desc}</p>
          </Card>
        ))} */}
